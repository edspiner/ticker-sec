...
var DEFAULT_WIDGET = 'widget',
    ...
    MW320_CHECK = !1,
    IDENTITY_TRANSFORM = !0 ? Ti.UI.create2DMatrix() : void 0,
    RESET = {
        bottom: null,
        left: null,
        right: null,
...
RESET = 1 ? _.extend(RESET, {
    ...
    focusable: !1,
    keepScreenOn: !1
}) : _.extend(RESET, { backgroundLeftCap: 0, backgroundTopCap: 0 });
...
exports.M = function(name, modelDesc, migrations) {
        var mod, config = (modelDesc || {}).config || {},
            adapter = config.adapter || {},
            extendObj = {},
            extendClass = {};
        adapter.type ? (mod = require('/alloy/sync/' + adapter.type), extendObj.sync = function(method, model, opts) {
    ...
    exports.C = function(name, modelDesc, model) {
        var mod, extendObj = { model: model },
            config = (model ? model.prototype.config : {}) || {};
        config.adapter && config.adapter.type ? (mod = require('/alloy/sync/' + config.adapter.type), extendObj.sync = function(method, model, opts) {
            return mod.sync(method, model, opts);
        }) : extendObj.sync = function(method, model, opts) {
    ...
    exports.UI.create = function(controller, apiName, opts) {
        ...
        var baseName, ns,
            parts = apiName.split('.');
        if (1 === parts.length) baseName = apiName,
            ns = opts.ns || CONST.IMPLICIT_NAMESPACES[baseName] || CONST.NAMESPACE_DEFAULT;
        else
        if (1 < parts.length) baseName = parts[parts.length - 1],
            ns = parts.slice(0, parts.length - 1).join('.');
        else throw 'Alloy.UI.create() failed: No API name was given in the second parameter';
    ...
    exports.createStyle = function(controller, opts, defaults) {
        ...
        var i, len, styleFinal = {};
        for (i = 0, len = styleArray.length; i < len; i++) {
            var style = styleArray[i],
                styleApi = style.key;
            if (style.isApi && -1 === styleApi.indexOf('.') && (styleApi = (CONST.IMPLICIT_NAMESPACES[styleApi] || CONST.NAMESPACE_DEFAULT) + '.' + styleApi), style.isId && opts.id && style.key === opts.id || style.isClass && _.contains(classes, style.key));
            else if (!style.isApi) continue;
            else if (-1 === style.key.indexOf('.') && (style.key = addNamespace(style.key)), style.key !== apiName) continue;
            style.queries && style.queries.formFactor && !exports[style.queries.formFactor] || style.queries && style.queries.if && ('false' === style.queries.if.trim().toLowerCase() || -1 !== style.queries.if.indexOf('Alloy.Globals') && !1 === exports.Globals[style.queries.if.split('.')[2]]) || exports.deepExtend(!0, styleFinal, style.style);
        }
...
exports.addClass = function(controller, proxy, classes, opts) {
        if (!classes) return void(opts && (MW320_CHECK, proxy.applyProperties(opts)));
        ...
        return (beforeLen === newClasses.length ? void(opts && (MW320_CHECK, proxy.applyProperties(opts))) : void processStyle(controller, proxy, newClasses, opts));
    },
    exports.removeClass = function(controller, proxy, classes, opts) {
        classes = classes || [];
        var pClasses = proxy[CONST.CLASS_PROPERTY] || [],
            beforeLen = pClasses.length;
        if (!beforeLen || !classes.length) return void(opts && (MW320_CHECK, proxy.applyProperties(opts)));
        classes = _.isString(classes) ? classes.split(/\s+/) : classes;
        var newClasses = _.difference(pClasses, classes);
        return (beforeLen === newClasses.length ? void(opts && (MW320_CHECK, proxy.applyProperties(opts))) : void processStyle(controller, proxy, newClasses, opts, RESET));
    },
...
exports.isTablet = function() {
        ...
        return 1 ? 1 ? isTabletFallback() : 1024 <= Math.max(Ti.Platform.displayCaps.platformHeight, Ti.Platform.displayCaps.platformWidth) : 400 <= Math.min(Ti.Platform.displayCaps.platformHeight, Ti.Platform.displayCaps.platformWidth);
    }(),
    ...
    exports.deepExtend = function() {
        var options, name, src, copy, copy_is_array, clone, target = arguments[0] || {},
            i = 1,
        ...
        for ('boolean' == typeof target && (deep = target, target = arguments[1] || {}, i = 2), 'object' == typeof target || _.isFunction(target) || (target = {}); i < length; i++)
            if (options = arguments[i], null != options)
                for (name in 'string' == typeof options && (options = options.split('')), options)(src = target[name], copy = options[name], target !== copy) && (deep && copy && !_.isFunction(copy) && _.isObject(copy) && ((copy_is_array = _.isArray(copy)) || !_.has(copy, 'apiName')) ? (copy_is_array ? (copy_is_array = !1, clone = src && _.isArray(src) ? src : []) : _.isDate(copy) ? clone = new Date(copy.valueOf()) : clone = src && _.isObject(src) ? src : {}, target[name] = exports.deepExtend(deep, clone, copy)) : target[name] = copy);
        return target;
    };