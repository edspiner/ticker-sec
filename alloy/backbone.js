...
})(this,function(root,Backbone,_,$){var
...
return events.push({callback:callback,context:context,ctx:context||this}),this;
},



once:function(name,callback,context){
if(!eventsApi(this,'once',name,[callback,context])||!callback)return this;var
self=this,
...
if(!eventsApi(this,'trigger',name,args))return this;var
...
for(var id in callback||'object'!=typeof name||(callback=this),obj&&((listeningTo={})[obj._listenId]=obj),listeningTo)
obj=listeningTo[id],
...
for(var names=name.split(eventSplitter),i=0,l=names.length;i<l;i++)
...
switch(args.length){
case 0:for(;++i<l;)(ev=events[i]).callback.call(ev.ctx);return;
case 1:for(;++i<l;)(ev=events[i]).callback.call(ev.ctx,a1);return;
case 2:for(;++i<l;)(ev=events[i]).callback.call(ev.ctx,a1,a2);return;
case 3:for(;++i<l;)(ev=events[i]).callback.call(ev.ctx,a1,a2,a3);return;
default:for(;++i<l;)(ev=events[i]).callback.apply(ev.ctx,args);return;}
...
Events[method]=function(obj,name,callback){var
...
for(attr in unset=options.unset,silent=options.silent,changes=[],changing=this._changing,this._changing=!0,changing||(this._previousAttributes=_.clone(this.attributes),this.changed={}),current=this.attributes,prev=this._previousAttributes,this.idAttribute in attrs&&(this.id=attrs[this.idAttribute]),attrs)
val=attrs[attr],
_.isEqual(current[attr],val)||changes.push(attr),
_.isEqual(prev[attr],val)?


delete this.changed[attr]:this.changed[attr]=val,
...
null==attr?!_.isEmpty(this.changed):
...
if(!diff)return!!this.hasChanged()&&_.clone(this.changed);var
val,changed=!1,
...
null!=attr&&this._previousAttributes?
...
return options.success=function(resp){return!!model.set(model.parse(resp,options),options)&&void(success&&success(model,resp,options),model.trigger('sync',model,resp,options))},wrapError(this,options),this.sync('read',this,options);
},
...
if(null==key||'object'==typeof key?(attrs=key,options=val):(attrs={})[key]=val,options=_.extend({validate:!0},options),attrs&&!options.wait){
if(!this.set(attrs,options))return!1;}else
...
return options.success=function(resp){model.attributes=attributes;var serverAttrs=model.parse(resp,options);return options.wait&&(serverAttrs=_.extend(attrs||{},serverAttrs)),(!_.isObject(serverAttrs)||model.set(serverAttrs,options))&&void(success&&success(model,resp,options),model.trigger('sync',model,resp,options))},wrapError(this,options),method=this.isNew()?'create':options.patch?'patch':'update','patch'===method&&(options.attrs=attrs),xhr=this.sync(method,this,options),attrs&&options.wait&&(this.attributes=attributes),xhr;
},
...
if(options.success=function(resp){(options.wait||model.isNew())&&destroy(),success&&success(model,resp,options),model.isNew()||model.trigger('sync',model,resp,options)},this.isNew())
...
var error=this.validationError=this.validate(attrs,options)||null;return!
error||(
this.trigger('invalid',this,error,_.extend(options,{validationError:error})),!1);
...
var singular=!_.isArray(models);
models=singular?models?[models]:[]:_.clone(models);var
i,l,id,model,attrs,existing,sort,
at=options.at,
...
if(attrs=models[i]||{},id=attrs instanceof Model?model=attrs:attrs[targetModel.prototype.idAttribute||'id'],existing=this.get(id))
...
this.models.splice(at+i,0,toAdd[i]);else
...
null==obj?void 0:
this._byId[obj]||this._byId[obj.id]||this._byId[obj.cid]);
},
...
where:function(attrs,first){return(
_.isEmpty(attrs)?first?void 0:[]:
...
return options.success=function(resp){var method=options.reset?'reset':'set';collection[method](resp,options),success&&success(collection,resp,options),collection.trigger('sync',collection,resp,options)},wrapError(this,options),this.sync('read',this,options);
},
...
options.wait||this.add(model,options);var
collection=this,
success=options.success;
...
return options.success=function(model,resp){options.wait&&collection.add(model,options),success&&success(model,resp,options)},model.save(null,options),model;
},
...
var model=new this.model(attrs,options);return(
model.validationError?(
this.trigger('invalid',this,model.validationError,options),!1):model);
...
if(_.isFunction(method)||(method=this[events[key]]),method){var
...
'GET'===params.type||options.emulateJSON||(
...
return Backbone.history.route(route,function(fragment){var args=router._extractParameters(route,fragment);router.execute(callback,args),router.trigger.apply(router,['route:'+name].concat(args)),router.trigger('route',name,args),Backbone.history.trigger('route',router,name,args)}),this;
},
...
if(this.routes){
this.routes=_.result(this,'routes');for(
...
return route=route.replace(escapeRegExp,'\\$&').replace(optionalParam,'(?:$1)?').replace(namedParam,function(match,optional){return optional?match:'([^/?]+)'}).replace(splatParam,'([^?]*?)'),new RegExp('^'+route+'(?:\\?([\\s\\S]*))?$');
},
...
return _.map(params,function(param,i){return(

i===params.length-1?param||null:
...
this._hasPushState=!!(this.options.pushState&&this.history&&this.history.pushState);var
...
this._hasPushState&&this.atRoot()&&loc.hash&&(
...
current===this.fragment&&this.iframe&&(
...
this.history[options.replace?'replaceState':'pushState']({},document.title,url);else



if(this._wantsHashChange)
this._updateHash(this.location,fragment,options.replace),
this.iframe&&fragment!==this.getFragment(this.getHash(this.iframe))&&(



!options.replace&&this.iframe.document.open().close(),
this._updateHash(this.iframe.location,fragment,options.replace));else





return this.location.assign(url);return(

options.trigger?this.loadUrl(fragment):void 0)}
},
...
var extend=function(protoProps,staticProps){var
...
child=protoProps&&_.has(protoProps,'constructor')?protoProps.constructor:
...
Model.extend=Collection.extend=Router.extend=View.extend=History.extend=extend;var
