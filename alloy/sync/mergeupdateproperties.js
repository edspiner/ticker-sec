function S4(){
return(0|65536*(1+Math.random())).toString(16).substring(1);
}

function guid(){
return S4()+S4()+"-"+S4()+"-"+S4()+"-"+S4()+"-"+S4()+S4()+S4();
}

function createMergedCopy(obj1,obj2){
for(var p in obj2)
try{

obj1[p]=obj2[p].constructor==Object?createMergedCopy(obj1[p],obj2[p]):

obj2[p];

}catch(e){
obj1[p]=obj2[p];
}

return obj1;
}

function Sync(method,model,opts){var
prefix=model.config.adapter.collection_name?model.config.adapter.collection_name:"default",
regex=new RegExp("^("+prefix+")\\-(.+)$"),
resp=null;
if("read"===method){
if(opts.parse){
var list=[];
_.each(TAP.listProperties(),function(prop){
var match=prop.match(regex);
null!==match&&list.push(TAP.getObject(prop));
}),
resp=list;
}else{
var obj=TAP.getObject(prefix+"-"+model.id);
model.set(obj),
resp=model.toJSON();
}}else
if("create"===method||"update"===method){
var mrg;
model.id?



mrg=TAP.getObject(prefix+"-"+model.id)||{}:(model.id=guid(),model.set(model.idAttribute,model.id));

var curr=model.toJSON();
mrg=createMergedCopy(mrg,curr),

TAP.setObject(prefix+"-"+model.id,mrg||{}),
resp=mrg;
}else"delete"===method&&(
TAP.removeProperty(prefix+"-"+model.id),
model.clear(),
resp=model.toJSON());

resp?(
_.isFunction(opts.success)&&opts.success(resp),
"read"===method&&model.trigger("fetch")):
_.isFunction(opts.error)&&opts.error(resp);
}

var Alloy=require("alloy"),
_=require("alloy/underscore")._,
TAP=Ti.App.Properties;

module.exports.sync=Sync,

module.exports.beforeModelCreate=function(config){




return config=config||{},config.columns=config.columns||{},config.defaults=config.defaults||{},("undefined"==typeof config.columns.id||null===config.columns.id)&&(config.columns.id="String"),config;
};